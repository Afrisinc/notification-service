...
export class AfricasTalkingProvider implements ISMSProvider {
  ...
  async send(smsData: any): Promise<{ messageId: string; provider: string }> {
    try {
      ...
      // Send SMS via Promise-based API
      ...
      // Use Promise-based API with timeout to prevent indefinite hanging
      const result = await Promise.race([
        this.smsService.send(sendPayload),
        new Promise((_, reject) =>
          setTimeout(() => reject(new Error("Africa's Talking API request timed out after 30 seconds")), 30000)
        ),
      ]);
