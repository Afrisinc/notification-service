...
export class TenantController {
  ...
  async createTenant(request: FastifyRequest, reply: FastifyReply) {
    try {
      const { code, name } = request.body as CreateTenantDTO;
      
      const tenant = await tenantDbService.createTenant({ code, name });

      logger.info({ tenantId: tenant.id, code: tenant.code }, "Tenant created");
  ...
  async createApiKey(request: FastifyRequest, reply: FastifyReply) {
    try {
      ...
      if (!name) {
        return ApiResponseHelper.missingFields(
          reply,
      ...
      logger.info(
        { tenantId: id, apiKeyId: apiKey.id },
        "API key created for tenant",
      );
