...
export async function listAppNotificationLogs(req: FastifyRequest, reply: FastifyReply) {
  ...
  const result = await notificationLogsService.listAppLogs(appId, orgId, {
...
export async function getAppNotificationLog(req: FastifyRequest, reply: FastifyReply) {
  const { orgId, appId, notificationId } = req.params as { orgId: string; appId: string; notificationId: string };

  const log = await notificationLogsService.getNotificationLog(appId, orgId, notificationId);
...
export async function exportNotificationLogs(req: FastifyRequest, reply: FastifyReply) {
  ...
  const logs = await notificationLogsService.getLogsForExport(appId, orgId, {
    status: query.status,
    channel: query.channel,
    dateFrom: query.dateFrom ? new Date(query.dateFrom) : undefined,
    dateTo: query.dateTo ? new Date(query.dateTo) : undefined,
