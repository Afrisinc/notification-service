...
export async function apiKeyMiddleware(request: FastifyRequest, reply: FastifyReply) {
  ...
  if (!authHeader) {
    logger.warn({ requestId: request.id }, 'Missing authorization header for API key');
  ...
  try {
    ...
    // Set account context from API key
    request.headers['x-account-id'] = validation.account_id;
    request.headers['x-api-key-id'] = validation.keyId;

    logger.debug({ requestId: request.id, account_id: validation.account_id }, 'API key validated');
  } catch (error) {
    logger.error(
      {
